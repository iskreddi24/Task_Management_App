...
// // function UrlFilePreview({ url, label, isImage = false }) {
// //   if (!url) return null;

// //   const fullUrl = `http://localhost:8081/uploads/${url}`;
...
// // const applyQuickRange = async (rangeFunc) => {
...
// //   // auto apply filters
// //   setTimeout(() => applyBackendFilters(), 100);
// // };
...
// //     return users.filter((u) => (u.departmentId || u.department?.id) === Number(deptId));
// //   };
...
// //       if (drawerTaskId === id) openDrawer(id);
// //     } catch (err) {
...
// //       const completedTime = new Date(t.completedAt || t.updatedAt).getTime();
// //       if (completedDateFilter === "TODAY") return completedTime >= getStartOfDay();
...
// //                     <button className="btn btn-primary" onClick={applyBackendFilters}>
// //                       Apply Filters
// //                     </button>
...
// //                           {task.assignee?.username} ({task.assignee?.role}) — {task.assignee?.departmentName || "N/A"}
// //                         </div>

// //                         <div>
// //                           <strong>Creator:</strong> {task.creator?.username} ({task.creator?.role}){" "}
// //                           {task.creator?.departmentName ? `— ${task.creator.departmentName}` : ""}
// //                         </div>
...
// //                               {!canEditTask(task) && task.creator?.email === user.email && (
// //                                 <p className="edit-disabled-text">You can edit only within 5 minutes of creation.</p>
// //                               )}
// //                             </div>
// //                           )}
// //                         </>
...
// //                         <UrlFilePreview url={task.attachmentUrl} label="📎Creator Attachment" />
// //                         <UrlFilePreview url={task.proofUrl} label="📁Proof File" isImage />
// //                       </div>
...
// //                               <input
// //                                 type="file"
// //                                 onChange={(e) => setProofFileMap({ ...proofFileMap, [task.id]: e.target.files[0] })}
// //                               />
// //                             )}

// //                             <button className="btn btn-purple" disabled={!proofFileMap[task.id]} onClick={() => handleSubmit(task.id)}>
// //                               Submit
// //                             </button>
// //                           </div>
// //                         )} */}
...
// //                               onChange={(e) => setReviewComments({ ...reviewComments, [task.id]: e.target.value })}
// //                             ></textarea>

// //                             <div className="review-actions">
// //                               <button className="btn btn-success" onClick={() => handleReview(task.id, "ACCEPT")}>
// //                                 ✓ Accept
// //                               </button>

// //                               <button className="btn btn-danger" onClick={() => handleReview(task.id, "REJECT")}>
// //                                 ✕ Reject
// //                               </button>
// //                             </div>
// //                           </div>
...
// //                         <button className="btn btn-ghost" onClick={() => openDrawer(task.id)}>
// //                           History
// //                         </button>
// //                       </div>
// //                     </article>
...
// //                       if (totalPages <= 7 || (pageNum >= start && pageNum <= end)) {
// //                         return (
// //                           <button key={i} className={i === currentPage ? "active" : ""} onClick={() => changePage(i)}>
// //                             {i + 1}
// //                           </button>
// //                         );
// //                       }

// //                       if (i === start - 1 || (i === end + 1 && end < totalPages - 1)) {
// //                         if ((i === start - 1 && start > 0) || (i === end + 1 && end < totalPages - 1)) {
// //                           return i === currentPage + 3 ? <span key={`ellipsis-${i}`}>...</span> : null;
// //                         }
// //                       }
...
// //           <div className="history-modal" style={{ top: modalPos.y, left: modalPos.x }} onMouseDown={startDrag}>
...
// //                         <a href={`http://localhost:8081/uploads/${h.fileUrl}`} target="_blank" rel="noreferrer">
// //                           📎 View File
// //                         </a>
// //                       </div>
// //                     )}
...
// //                 ) : (
// //                   <FilePreview file={editForm.file} onRemove={() => setEditForm({ ...editForm, file: null })} />
// //                 )}

// //                 {editForm.currentAttachmentUrl && <UrlFilePreview url={editForm.currentAttachmentUrl} label="Current File" />}
// //               </div>
...
// //                     {departments
// //                       .filter((d) => user.role === "SUPER_ADMIN" || getDeptId(user) === d.id)
// //                       .flatMap((d) =>
...
// //                 Save Changes
// //               </button>
// //               <button className="btn btn-ghost" onClick={closeEditPopup}>
// //                 Cancel
// //               </button>
// //             </div>
// //           </div>
...
//     // 💥 reset modal to center every time
//     setModalPos({
//       x: window.innerWidth / 2 - 275, // 550px modal width / 2
...
// // ⚡ QUICK DATE FILTER HANDLER (MUST be inside component)
                ...
                drawerHistory.map((h, i) => (
                  <div className="history-item" key={i}>
                    ...
                    {h.fileUrl && (
                      <div className="history-file">
                        <a
                          href={`http://localhost:8081/uploads/${h.fileUrl}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          📎 View File
